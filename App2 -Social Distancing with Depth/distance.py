...
class DistanceGuardianDebug(DistanceGuardian):
    def parse_frame(self, frame, boxes):
        ...
        for result in results:
            x1 = int((result['detection1']["x_min"] + (result['detection1']["x_max"] - result['detection1']["x_min"]) /
                      2) *
                     img_w)
            y1 = int(result['detection1']["y_max"] * img_h)
            x2 = int((result['detection2']["x_min"] + (result['detection2']["x_max"] - result['detection2']["x_min"]) /
                      2) *
                     img_w)
