    ...
    def capture(self):
        for frame, detections in super().capture():
            ...
            for detection in detections:
                ...
                cv2.putText(frame, "x: {}".format(round(detection["depth_x"], 1)), (left, top + 30),
                            cv2.FONT_HERSHEY_TRIPLEX, 0.5, 255)
                cv2.putText(frame, "y: {}".format(round(detection["depth_y"], 1)), (left, top + 50),
                            cv2.FONT_HERSHEY_TRIPLEX, 0.5, 255)
                cv2.putText(frame, "z: {}".format(round(detection["depth_z"], 1)), (left, top + 70),
                            cv2.FONT_HERSHEY_TRIPLEX, 0.5, 255)
                cv2.putText(frame, "conf: {}".format(round(detection["conf"], 1)), (left, top
                                                                                       + 90), cv2.FONT_HERSHEY_TRIPLEX, 0.5, 255)
