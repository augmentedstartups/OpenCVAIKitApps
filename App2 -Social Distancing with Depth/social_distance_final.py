...
# Social Distancinging App
# www.augmentedstartups.info/YouTube
import logging
import math
...
audio_file_path = os.path.abspath("./no.mp3")
...
def schedule_task_():
    global task_play_sound
    task_play_sound = True
    threading.Timer(10, schedule_task_).start()
...
class Main:
    ...
    def parse_frame(self, frame, results):
        ...
        if should_alert:
            img_h = frame.shape[0]
            img_w = frame.shape[1]
            cv2.putText(frame, "Too close", (int(img_w / 3), int(img_h / 2)), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 1)
            global task_play_sound
            if task_play_sound:
                task_play_sound = False
                threading.Thread(target=play_sound, args=(audio_file_path,)).start()
